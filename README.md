...
- ⏰ **开抢前提醒**：消费券这类「有时刻的事件」不等第二天日报——开抢前 45 分钟提醒一次，
  迟到 15 分钟内仍算数，每个事件只提醒一次，一天最多两条
...
| `notify.event.max_per_day` | 一天最多几条提醒（每个事件至多 1 次） | 2 |
...
消费券、限量秒杀这类东西的关键不是"值不值"，而是**几点开始**。公告通常提前几天发布，
...
⏰ 2 项即将开抢                            ← 同一轮多条：抬头只放一个时刻，
· 09月21日 10:00 开抢 · 洪城消费券第一轮       所以每一条在正文里带上自己的时刻
· 09月22日 10:00 开抢 · 洪城消费券第二轮
Deal-Hunter · 09-21 09:35
```

三条规则值得单独说，因为它们决定了"提醒"到底可不可信：
...
- [~] **本地消费券**：抓官方发布页、解析开抢时间、开抢前提醒 —— 代码与测试已完成，
  待部署并按下一条真实公告复核（设计见 [docs/superpowers/specs/](docs/superpowers/specs/)）
