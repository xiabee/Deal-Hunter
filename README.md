...
#    还没接入的候选地址也能量（信源准入那道题问的就是这个，不必先改生产配置）
./dist/dealhunter probe -url https://example.gov.cn/xxgk/tzgg/index.shtml -kind html -trust 7
...
📊 127.0.0.1:8765
 ...
 └─ 🩺 信息源健康度  命中/入库/耗时/错误 + 官方源标记 + 每个源"上次出声是多久前"
```
...
配色有三态，点右上角的芯片循环：**跟随系统 → 日间 → 夜间 → 跟随系统**。
默认那态什么都不存，系统切深色面板当场跟着切；手动选过的那次会记在 `localStorage`，
再点回「跟随系统」就把记录撤掉，不会从此和系统脱钩。
...
| 是不是能领 | `Deal.Claimable()`：带免费/折扣/试用/额度/券信号，且不是提问、吐槽或**向人求助**（`求解答 / 求指导 / 请教 / 求推荐 / 帮我看看`）。`请教` 要求前面不是"申"——「免费申**请教**程」是"申请 + 教程"，是生产流量上最常见的误伤 |
...
- **两种写法都能读出结束**：带前置词的「核销期限至2026年10月15日」，以及**起止区间**
  ...
  区间也可以只写尾部的日：「9月20日至27日」按前一个日期补上月份，尾部日子不比开头大就
  不认（「9月20日至10月3日」不能读成 9 月 3 日）；同一句里既有「截止」又有区间时，
  以「截止」那个日期为准。
  ...
  这种"年写全 + 前"的说法也认。**不写年份的日期按这条公告自己的日期补年**：
  `限时免费至9月10日` 发在 8 月 4 日 → 读成 2026-09-10；同样这句话出现在 9 月 21 日的
  帖子里就是"已经过了"，不读。跨年只允许在年底附近（`12月30日` 的帖子写 `截止至1月5日`
  成立），且滚完必须在原日期之后 45 天以内 —— 宁可少读，不猜一个错的年。
  日期后面紧贴着写了点就按点记（`截止到9月20日凌晨5:40` → 05:40，不再按当天 23:59 收尾，
  免得一张早上就结束的券在日报里多活到深夜）；逗号之后是另一句话，不会把"10:00 开抢"
  借来当截止时刻。
- **公告只说时长、不写日期时也读**：`限时两周 / 限免 3 天 / 限时免费一周 / 为期两周` 从这条
  公告自己的日期起算，按天数的记到当天 23:59:59，按小时的记到点。三条限制是为不误伤加的：
  只有 `限时 / 限免 / 为期` 三个头部词算（`试用一个月` 是试用时长、`3天会员卡` 是卡本身的有效期，
  都不说活动何时结束）；**同一段文字里出现任何写全的日期就整个让路**（`限免两周至9月10日` 若按两周
  推会把 9 月 10 日推成 9 月 4 日）；汉字数字只认单字，`限时三十天` 认不出就整条不读，
  绝不读成"十天"。
...
dealhunter [全局参数] <命令>
  ...
  serve         只开面板不采集
  probe         单源探测（-source 已配置的源 / -url 还没接入的候选）：报出每行会被生产怎么用
                （分数、时刻、挡下的原因），不写库、不推送
  sources       列出信息源
  deals         查看最近入库（-min 按分数筛着看）
  daily         立刻发送今天的日报（占用当天那一份）；-dry 只列出
  events        列出正在跟踪的限时事件（开抢 / 截止两端，各自状态）
  notify-test   全通道链路自检
  ...
  version       打印版本（commit 与构建时间戳，部署核验就认这个）
```

> **为什么要 `reparse`**：信源的游标只往前走，一条公告被采过之后就不会再回来，所以**解析器的修复
> 追不上已经躺在库里的行**。补完措辞、跑一次 `reparse`，旧行才拿得到新时刻（只补时刻，不重算分数——
> 否则几个月前的发现会因为涨分挤进明天的日报）。读不出时刻时保留原值：把已写的截止抹掉，
> 等于让一场已经结束的活动重新出现在日报里。
...
> **测试一律不许出门**：`doctor` 的 `-net` 默认是 `true`，会逐个探测全部信源 —— 带上它就是一次
> 100 秒开外的真实外连，网络一慢测试直接超时（2026-09-21 就这样在门禁里挂满 600 秒）。
> 跑 `doctor` 的测试必须写 `-net=false`（现有几条都是），整包因此从分钟级回到 0.15 秒。
...
`doctor` 还回答两件同样"没人记得就等于没做"的事：**上次真剪过是什么时候**（`compaction`），
以及**当前解析器是否比库里读得更多**（`reparse drift`）。后者是 `reparse` 存在的理由——信源游标只往前走，
误判过的公告不会被下一轮再读一次，只有 `reparse` 会去读第二次；漂移一出现 doctor 就点名，
且它报的行数与 `dealhunter reparse` 的 dry-run 一致（有测试钉住两边必须同数）。
写回仍然是人的决定，不进排程：覆盖一条**已经写明**的截止时刻有可能把真值改错，
这与"补一个空字段"不是同一风险等级。
