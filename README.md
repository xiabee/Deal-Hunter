...
```
📊 127.0.0.1:8765
 ├─ KPI   累计发现 · 已推送 · 待推送 · 信息源 15/15 · 本轮用时 · 链接成色 25%
 ├─ 🧭 飞书 ● app · 下一轮 29m · 最后同步 17:35:16
 ├─ 📈 情报流  一行一条：分数 · 标题(官方页优先) · ✅/⚠️ · 相对时间 · 「详情」
 │             展开后才是摘要、徽章、评分理由、原始出处与厂商官网入口
 ├─ 🕒 采集记录  最近 8 轮的新增/推送/静默/官方定位/错误与耗时（读 /api/v1/runs）
...
零成本的判定（厂商自有域名 / 根本认不出厂商）对**每一条入库记录**生效，联网的两级只对本轮准备
推送的少数几条做。所以「没有徽章」= 知道厂商但还没去核实，而不是「已核实为第三方」。

---

## 📬 推送长什么样

只推结论，细节留在面板：一条时给一句摘要 + 按钮，一批时一行一条，锁屏上能扫完。

```text
🆓 免费 · 智谱 GLM-5.3-flash 限时免费开放        ← 单条：标题进抬头
置信分 92 · 智谱AI · ✅ 官方页已校验
> 官方公告：API 调用 0 元。
[ 官方入口 ] [ 原始出处 ]
...
分段的理由是：告警负责"别错过"，早报负责"别重复读"。把三天前就看过的免费额度和
今早刚收录的新活动混在一张列表里，读者每次都要重扫一遍才知道有没有新东西。
...
挡在早报之外——早报回答的是"今天还能领什么"，不是"大家在讨论什么"。
