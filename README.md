...
| 信息源可信度 | +0…+10 | 自主探测给满分 |
| 链接已定位到官方页 | **+8** | 见下一节：厂商入口页校验通过 |
| 只有第三方转述 | −6 | 检索正常但找不到厂商页（服务不可用时不扣分） |

---

### 🔗 链接校准：优先给官方入口

社区帖说「XX 免费」不等于你能在 XX 官网上点到。每条准备推送的羊毛都会走一遍
**由便宜到昂贵的四级阶梯**，只有同时满足「域名属于该厂商」+「重定向后返回 2xx」
才允许改写链接：

```mermaid
flowchart TD
    A["一条待推送的羊毛"] --> B{"1️⃣ 链接本身就在<br/>厂商自有域名上？"}
    B -- 是 --> B1["✅ official<br/>零网络开销"]
    B -- 否 --> C{"2️⃣ 内置厂商入口页<br/>（精选，非爬取）"}
    C -- 探测 2xx --> C1["✅ vendor_entry<br/>改写为官方入口"]
    C -- 失败 --> D{"3️⃣ site: 定向检索<br/>厂商自己的域名"}
    D -- 命中且 2xx --> D1["✅ search_verified"]
    D -- 未命中 --> E["⚠️ third_party<br/>保留原链接并标注"]
    B1 --> F["卡片主按钮：官方入口（已校验）<br/>次按钮：原始出处"]
    C1 --> F
    D1 --> F
    E --> G["卡片如实写明「第三方转述」"]
```
...
| 不无限外连 | 每轮预算 `filter.max_official_lookups`（默认 10 次），用不完不补，用完如实标注 |
| 不反复打扰上游 | 判定结果缓存 7 天；厂商自有域名一条都不查（阶梯 1 直接命中） |
| 不掩盖出处 | `d.URL` 永不被覆盖，官方链接写在 `meta.official_url`，原始帖在 `meta.original_url` |

面板新增 **✅ 已定位官方页** 过滤器，`/api/v1/deals` 与日报、OpenClaw 拉取都读同一份
已校验结果；`/api/v1/status` 的 `last_run.verified_official / third_party_links` 说明本轮的成色。

零成本的判定（阶梯 1 与「认不出厂商」）会对**每一条入库记录**生效，所以面板里的徽章和卡片里
含义一致；需要联网的 2️⃣3️⃣ 级只对本轮准备推送的少数几条做。没有徽章 = 「知道厂商但还没去核实」，
不是「已核实为第三方」。
