...
2. **四件要人判的事**：清单/汇总类条目要不要继续进日报（量过：在效候选里 32 条）；备份要不要
   ...
   第五件（2026-09-23 量出来的）：**`hn-free-api` / `hn-ai-promo` 要不要放宽查询**。这两条上线以来
   一条都没交付过，原因不是坏了：现在的 `query="free API credits"` + `points>10` 在 30 条结果里
   0 条落在 14 天窗口内（换 `search_by_date` 也一样，最新仍是 20 天前，见 ROADMAP「已实测否决的方向」）。
   改成 `free API` 同样分数门槛 → 14 天内 10 条；`AI free` → 17 条。要不要改是**内容判断**（放宽会带进
   更多和"羊毛"无关的帖子），不是我该替你定的，所以只把价格摆在这里。
   另外体检的 `delivery` 行会在 **09-28** 之后从 ✓ 变 !（观察记号 09-21 起满 7 天），点名 5 个零交付源；
   三条已各自量过、都有确定解释：两条 HN（上面这件）、两条南昌（M51：区县门户 403、`tzgg` 当天全是政务要闻）、
   `copilot-plans-snapshot`（快照源只在价格事实变化时开口，基线打过之后就该是静默 —— 它 09-23 被
   `probe -url` 量到时反而报 3 行"收"，因为候选模式没有它的基线可比，别把那次读数当"这源能用"）。
...
| 项 | 结论 | 证据 |
|---|---|---|
| 候选地址可以直接 probe，不必先动生产配置（M50） | VERIFIED（生产实跑 + 四次变异） | `probe -url <地址> -kind <采集器> -trust <0-10>` 走 `sources.New` + `pipeline.Screen` 同一套；`-source`/`-url` 互斥，都不给或都给退 2。"候选拿不到官方域加成"这条是**成对判据**：同批字节走配置路径 + 被背书的域，分数正好差那个加成。**踩过的坑留给下次**：trust 取 8 时两边都顶到评分上限 100，delta 变成 9——那测的是 clamp 不是加成，夹具的分数量要靠 ceiling 之下。生产实测（09-23）：`tzgg` 首页作候选报 40 行、全部"无优惠命中词"，而同一个 URL 作为已配置源（带 `param` 收窄）只有 1 行——所以 `-url` 与 `-source` 是两个不同的问题，都得留 |
| 升级失败会退回上一版而不是把坏产物留在 live 路径（M48） | VERIFIED（真机部署 + 五次变异，两腿自修） | 见 ROADMAP M48。三条要留给下次的事实：① `rollback-prev.sh` 搬的是 root 所有、systemd 托管的文件，**手跑必须 sudo**（今晚以 `life` 跑，version 探测与判定都对，mv 处 `Permission denied`）；② "造一个没有 systemctl 的 PATH" 这招在 MSYS 上不成立（复制来的 PE 加载不到自己的 DLL），Linux 上成立，所以这一腿带前提探针、两台上各自如实报告；③ 我今晚**又**把 `install.sh` 平铺进了临时目录一次（`scp -r deploy/. :/tmp/X/` 而不是 `:/tmp/X/deploy/`），撞的就是本文那条笔记——从此传输先 `ls -1 . deploy` 确认布局再跑安装器，别相信"上次记过就不会再犯"。注：脚本头那行 `sudo` 说明是本次部署**之后**改的（注释级），装在主机上的那份字节略有出入，下次部署自然带过去 |
