...
1. 真实复核"到时候收到且只收到一次"（开抢与到期共用同一条通道、同一个每行标记）。
   9-21 那天的判断"生产库里 0 行带 `expires_at` / `starts_at`，所以无从触发"**已经过时**：现在
   721 条去重记录里有 4 行带时刻，`dealhunter events` 正在跟踪两条且都过了 60 分门槛，只是都还远：
   `09-30 23:59 截止`（77 分，到期提醒窗口 = 北京 09-30 20:59 起）与 `10-20 06:00 开抢`（63 分，
   窗口 = 北京 10-20 05:15–06:15）。也就是说这件不再是"等一条现实公告"，而是**等这两个时刻**——
   到点各查一次即可，判据与取数命令在 STATUS 的「复核判据」。
