...
1. **无年份的截止要按发布日补年份**（真实语料里剩下的最大一类）：`截止至8月28日`、
   `限时免费至9月10日` 这类中文公告不写年，现在一律读不出。可行做法是给 `ExpiresAt` 一个
   锚点（发布日期），只在"发布日期 ≤ 该日 ≤ 发布日期+18 个月"时接受，否则仍判读不出 ——
   宁缺毋滥这条不变。需要先定的是那个跨度取多少。
2. **区县/省级栏目按「信源准入」流程实测后再加**（M3 已给出做法与判据；今天量过的市级栏目
   不含券样本，见下表）。
