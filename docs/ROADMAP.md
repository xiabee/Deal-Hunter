...
| M9（语料回灌） | 拿生产库 1040 行真实公告文本喂回解析器量覆盖率：读出截止从 5 行提到 24 行，补的是年份明写的 `开放至/持续至/顺延至…` 与"年 + 前"后缀；配了反向守卫防把发布日读成截止 | 本次提交 |
| M10（解析补全） | 无年份的截止按公告自身日期补年（年底附近才允许滚一年，上限 45 天）；日期后紧贴的点被采信（`凌晨5:40` 不再按 23:59 收尾），逗号后的另一句不会被盗用 | 本次提交 |
