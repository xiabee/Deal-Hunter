...
1. 生产配置加上验证过的南昌信源（`swj.nc.gov.cn/ncsswj/tzgg/index.shtml`、
   `www.nc.gov.cn/ncszf/tzgg/2021_nav_list.shtml`）并部署；注意 `/ncszf/tzgg/` 会 307 跳首页，
   必须配具体列表页。
...
1. **面板在采集轮进行中不再被锁死**（P2，读 M2 代码时发现，未修）
...
2. **到期前提醒**：日报已展示截止日，缺「明天到期」单独一条 —— 与 M3 共用 `starts_at` 与事件调度，
   M3 收尾后接着做最省事。
3. **更多官方目录差分**：推理云平台 / 向量库 / GPU 云，复用 `openrouter` 的游标差分套路。
4. **ARM64 与真实部署验证**：交叉编译已过，但从未在 linux/arm64 与 systemd 下实跑过（见 Known Risks）。
5. **把身份门禁扩到文件内容**（小改动，价值高）：`check-identity.sh` 只查提交作者，`internal/secretlint`
...
6. **`ExpiresAt` 的时区**：它用 `time.Local` 构造时刻而比较用绝对时间，服务跑 UTC 时北京的截止日会
